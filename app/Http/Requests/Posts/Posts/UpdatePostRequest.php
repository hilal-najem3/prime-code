...
namespace App\Http\Requests\Posts\Posts;
...
class UpdatePostRequest extends FormRequest
{
    ...
    {
        $postId = $this->route('post'); // This assumes route model binding or {post} param
        ...
        $mediaIds = $post->media->where('id', '!=', $post->thumbnail_id)->pluck('id')->toArray();

        return array_merge([
            'link' => ['required', 'string', 'max:255', 'alpha_dash', "unique:posts,link,{$postId}"],

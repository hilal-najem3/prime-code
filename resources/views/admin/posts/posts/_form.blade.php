...
<form method="POST" action="{{ $isEdit ? route('admin.posts.update', $post->id) : route('admin.posts.store') }}"
    ...
    <x-form.select-box label="Categories" name="categories" :options="$categories->pluck('name', 'id')"
        :selected="old('categories', $post?->categories->pluck('id')->toArray() ?? [])" :multiSelect="true" />
    @else
    <x-form.select-box label="Categories" name="categories" :options="$categories->pluck('name', 'id')"
    ...
    <x-form.select-box label="Tags" name="tags" :options="$tags->pluck('name', 'id')"
        :selected="old('tags', $post?->tags->pluck('id')->toArray() ?? [])" :multiSelect="true" />
    @else
    ...
    <x-form.select label="Status" name="is_active" :options="['1' => 'Active', '0' => 'Inactive']"
        :selected="old('is_active', $post->is_active ?? '1')" />

    {{-- Featured --}}
    <x-form.select label="Featured" name="is_featured" :options="['1' => 'Featured', '0' => 'Not Featured']"
        :selected="old('is_featured', $post->is_featured ?? '1')" />

    <div class="flex justify-between pt-4">
        <a href="{{ route('admin.posts.index') }}" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">← Back</a>
